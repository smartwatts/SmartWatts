#!/usr/bin/env python3
"""
Train Anomaly Detection Model
Uses Autoencoder for detecting anomalies in energy consumption patterns
"""
import argparse
import sys
import os
from pathlib import Path
import logging
import numpy as np
import tensorflow as tf
from tensorflow import keras
from tensorflow.keras import layers, models, callbacks
import json
from datetime import datetime

# Add parent directory to path
sys.path.insert(0, str(Path(__file__).parent.parent))

from utils.data_loader import EnergyDataLoader

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


def build_anomaly_detection_model(input_shape: tuple) -> keras.Model:
    """Build Autoencoder-based anomaly detection model"""
    # Encoder
    encoder = models.Sequential([
        layers.Dense(64, activation='relu', input_shape=input_shape),
        layers.Dropout(0.2),
        layers.Dense(32, activation='relu'),
        layers.Dropout(0.2),
        layers.Dense(16, activation='relu'),  # Bottleneck
    ], name='encoder')
    
    # Decoder
    decoder = models.Sequential([
        layers.Dense(32, activation='relu', input_shape=(16,)),
        layers.Dropout(0.2),
        layers.Dense(64, activation='relu'),
        layers.Dropout(0.2),
        layers.Dense(input_shape[0], activation='linear')  # Reconstruct input
    ], name='decoder')
    
    # Autoencoder
    autoencoder = models.Sequential([encoder, decoder], name='autoencoder')
    
    autoencoder.compile(
        optimizer=keras.optimizers.Adam(learning_rate=0.001),
        loss='mse',
        metrics=['mae']
    )
    
    return autoencoder, encoder


def train_model(args):
    """Main training function"""
    logger.info("Starting anomaly detection model training...")
    
    # Setup paths
    data_dir = Path(args.data_dir)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Load and prepare data
    logger.info("Loading and preprocessing data...")
    data_loader = EnergyDataLoader(data_dir, sequence_length=args.sequence_length)
    
    # Find data files
    data_files = list(data_dir.glob("*.csv"))
    if not data_files:
        logger.error(f"No CSV files found in {data_dir}")
        return
    
    # Load and combine data
    import pandas as pd
    dfs = []
    for file in data_files:
        df = data_loader.load_energy_data(str(file))
        dfs.append(df)
    
    combined_df = pd.concat(dfs, ignore_index=True)
    combined_df = combined_df.sort_values('timestamp')
    
    # Prepare data (use only normal data for training)
    X, y = data_loader.prepare_anomaly_data(combined_df, target_col='consumption')
    
    # Filter to normal data only (y == 0) for training
    normal_indices = np.where(y.flatten() == 0)[0]
    X_normal = X[normal_indices]
    
    # Split data
    X_train, X_val, X_test, _, _, _ = data_loader.split_data(X_normal, X_normal)
    
    logger.info(f"Training samples: {len(X_train)}")
    logger.info(f"Validation samples: {len(X_val)}")
    logger.info(f"Test samples: {len(X_test)}")
    
    # Build model
    logger.info("Building model...")
    autoencoder, encoder = build_anomaly_detection_model(input_shape=(X_train.shape[1],))
    
    # Callbacks
    checkpoint_cb = callbacks.ModelCheckpoint(
        str(output_dir / "anomaly_detection_best.h5"),
        save_best_only=True,
        monitor='val_loss',
        mode='min'
    )
    
    early_stopping_cb = callbacks.EarlyStopping(
        monitor='val_loss',
        patience=20,
        restore_best_weights=True
    )
    
    # Train model (autoencoder learns to reconstruct normal patterns)
    logger.info("Training model...")
    history = autoencoder.fit(
        X_train, X_train,  # Autoencoder: input = target
        validation_data=(X_val, X_val),
        epochs=args.epochs,
        batch_size=args.batch_size,
        callbacks=[checkpoint_cb, early_stopping_cb],
        verbose=1
    )
    
    # Evaluate on test set
    logger.info("Evaluating on test set...")
    test_loss, test_mae = autoencoder.evaluate(X_test, X_test, verbose=0)
    logger.info(f"Test Loss: {test_loss:.4f}, Test MAE: {test_mae:.4f}")
    
    # Calculate reconstruction threshold (for anomaly detection)
    train_predictions = autoencoder.predict(X_train, verbose=0)
    train_errors = np.mean(np.square(X_train - train_predictions), axis=1)
    threshold = np.percentile(train_errors, 95)  # 95th percentile as threshold
    logger.info(f"Anomaly detection threshold: {threshold:.4f}")
    
    # Save models
    autoencoder_path = output_dir / "anomaly_detection_model.h5"
    autoencoder.save(str(autoencoder_path))
    logger.info(f"Autoencoder saved to {autoencoder_path}")
    
    encoder_path = output_dir / "anomaly_detection_encoder.h5"
    encoder.save(str(encoder_path))
    logger.info(f"Encoder saved to {encoder_path}")
    
    # Save training history
    history_path = output_dir / "anomaly_detection_history.json"
    with open(history_path, 'w') as f:
        json.dump({k: [float(v) for v in values] for k, values in history.history.items()}, f, indent=2)
    
    # Save metadata
    metadata = {
        "model_name": "anomaly_detection",
        "version": "1.0.0",
        "trained_at": datetime.now().isoformat(),
        "sequence_length": args.sequence_length,
        "epochs": len(history.history['loss']),
        "test_loss": float(test_loss),
        "test_mae": float(test_mae),
        "threshold": float(threshold),
        "input_shape": list(X_train.shape[1:]),
        "output_shape": list(X_train.shape[1:])
    }
    
    metadata_path = output_dir / "anomaly_detection_metadata.json"
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=2)
    
    logger.info("Training completed successfully!")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Anomaly Detection Model")
    parser.add_argument("--data-dir", type=str, required=True, help="Directory containing training data")
    parser.add_argument("--output-dir", type=str, default="models", help="Output directory for models")
    parser.add_argument("--epochs", type=int, default=100, help="Number of training epochs")
    parser.add_argument("--batch-size", type=int, default=32, help="Batch size")
    parser.add_argument("--sequence-length", type=int, default=24, help="Input sequence length")
    
    args = parser.parse_args()
    train_model(args)

